CREATE DATABASE IF NOT EXISTS home_work_4_dop;
USE home_work_4_dop;
...
/*SELECT
	a1.mark,
	COUNT(a1.mark),
    (SELECT COUNT(a2.color) FROM auto a2 WHERE a2.mark NOT IN ("LADA"))
FROM auto a1
RIGHT JOIN auto a2 ON a1.regnum = a2.regnum
WHERE a1.mark IN ("LADA")
-- HAVING COUNT(a1.mark) IS NULL
-- GROUP BY mark*/
...
SELECT
	mark,
    color,
	COUNT(mark) OVER (PARTITION BY color) AS count
FROM auto
GROUP BY mark,color;
